...
class TestDefaultMode(Base):
    def _state(self):
        files = S.discover_files(self.root)
        modes = S.collect_modes(files)
        return files, modes, S.effective_modes(files, modes)

    def test_collect_ignores_missing_and_bad_values(self):
        write(self.gfile(), {"permissions": {"defaultMode": "acceptEdits", "allow": ["Read"]}})
        write(self.pfile("p1", "settings.json"), {"permissions": {"allow": ["Read"]}})  # none set
        write(self.pfile("p2", "settings.json"), {"permissions": {"defaultMode": 42}})  # not a str
        write(self.pfile("p3", "settings.json"), {"permissions": {"defaultMode": "bogus"}})
        _, modes, _ = self._state()
        by_proj = {m["project"]: m for m in modes}
        self.assertEqual(set(by_proj), {"(global)", "p3"})
        self.assertTrue(by_proj["(global)"]["valid"])
        self.assertFalse(by_proj["p3"]["valid"])   # surfaced, but flagged

    def test_builtin_default_when_nothing_set(self):
        write(self.gfile(), {"permissions": {"allow": ["Read"]}})
        write(self.pfile("p1", "settings.json"), {"permissions": {"allow": ["Read"]}})
        _, _, eff = self._state()
        p1 = next(e for e in eff if e["project"] == "p1")
        self.assertEqual(p1["mode"], "default")
        self.assertFalse(p1["explicit"])
        self.assertEqual(p1["shadowed"], [])

    def test_project_overrides_global_and_local_overrides_shared(self):
        write(self.gfile("settings.json"), {"permissions": {"defaultMode": "plan"}})
        write(self.pfile("p1", "settings.json"), {"permissions": {"defaultMode": "acceptEdits"}})
        write(self.pfile("p1", "settings.local.json"), {"permissions": {"defaultMode": "default"}})
        write(self.pfile("p2", "settings.json"), {"permissions": {"allow": ["Read"]}})
        _, _, eff = self._state()
        by = {e["project"]: e for e in eff}
        # project-local (rank 40) beats project-shared (30) beats user (10)
        self.assertEqual(by["p1"]["mode"], "default")
        self.assertEqual(by["p1"]["from_scope"], "project-local")
        self.assertEqual([s["mode"] for s in by["p1"]["shadowed"]], ["acceptEdits", "plan"])
        # p1's mode must not leak to p2, which falls back to the global one
        self.assertEqual(by["p2"]["mode"], "plan")
        self.assertEqual(by["p2"]["from_scope"], "user")
        # and the no-project case sees the global too
        self.assertEqual(by["(global)"]["mode"], "plan")

    def test_user_local_outranks_user(self):
        write(self.gfile("settings.json"), {"permissions": {"defaultMode": "plan"}})
        write(self.gfile("settings.local.json"), {"permissions": {"defaultMode": "acceptEdits"}})
        _, _, eff = self._state()
        g = next(e for e in eff if e["project"] == "(global)")
        self.assertEqual(g["mode"], "acceptEdits")
        self.assertEqual(g["from_scope"], "user-local")

    def test_set_and_clear_preserve_other_keys(self):
        f = self.gfile()
        write(f, {"permissions": {"allow": ["Read"]}, "model": "opus"})
        ok, _ = S.set_default_mode(f, "plan")
        self.assertTrue(ok)
        d = S.load_json(f)
        self.assertEqual(d["permissions"]["defaultMode"], "plan")
        self.assertEqual(d["permissions"]["allow"], ["Read"])   # rules untouched
        self.assertEqual(d["model"], "opus")                    # other keys untouched
        # clearing removes just the key
        ok, _ = S.set_default_mode(f, "")
        self.assertTrue(ok)
        d = S.load_json(f)
        self.assertNotIn("defaultMode", d["permissions"])
        self.assertEqual(d["permissions"]["allow"], ["Read"])
        # clearing again is an error, not a silent success
        self.assertFalse(S.set_default_mode(f, "")[0])

    def test_set_rejects_invalid_mode(self):
        f = self.gfile()
        write(f, {"permissions": {"defaultMode": "plan"}})
        ok, info = S.set_default_mode(f, "acceptAllTheThings")
        self.assertFalse(ok)
        self.assertIn("invalid", info)
        self.assertEqual(S.load_json(f)["permissions"]["defaultMode"], "plan")  # unchanged

    def test_set_creates_missing_file(self):
        f = self.pfile("brand-new", "settings.json")
        ok, _ = S.set_default_mode(f, "acceptEdits")
        self.assertTrue(ok)
        self.assertEqual(S.load_json(f)["permissions"]["defaultMode"], "acceptEdits")

    def test_suggestions_flag_shadowed_invalid_and_risky(self):
        write(self.gfile(), {"permissions": {"defaultMode": "plan"}})
        write(self.pfile("p1", "settings.json"), {"permissions": {"defaultMode": "bypassPermissions"}})
        write(self.pfile("p2", "settings.json"), {"permissions": {"defaultMode": "nope"}})
        files, modes, eff = self._state()
        sg = S.build_suggestions(S.collect_rules(files), modes, eff)
        kinds = [s["kind"] for s in sg]
        self.assertIn("mode-invalid", kinds)
        self.assertIn("mode-shadowed", kinds)   # global 'plan' is dead for p1
        risky = [s for s in sg if s["kind"] == "mode-risky"]
        self.assertTrue(any("bypassPermissions" in s["title"] for s in risky))

    def test_suggestions_backwards_compatible(self):
        write(self.gfile(), {"permissions": {"allow": ["Read"]}})
        S.build_suggestions(S.collect_rules(S.discover_files(self.root)))  # no mode args
